...
@Component({
  selector: 'app-novi-dekorater',
  templateUrl: './novi-dekorater.component.html',
  styleUrls: ['./novi-dekorater.component.css']
})
export class NoviDekoraterComponent {
  ...
  telefonReg: string = '';
  emailReg: string = '';
  profilnaSlikaReg: string = '../../assets/defaultImage-100x100.jpg';
  brojKarticeReg: string = '';
  errorReg: string = '';

  message: string = '';

  // defaultImage: string = '../../assets/defaultImage-100x100.jpg';
      ...
      reader.onload = (e: any) => {
        ...
        img.onload = () => {
          if (img.width >= 100 && img.width <= 300 && img.height >= 100 && img.height <= 300) {
            ...
            this.profilnaSlikaReg = '../../assets/defaultImage-100x100.jpg'; // Resetovanje prikaza slike
          }
        };
      };
  ...
  addNewDecorator() {
    ...
    let isCardValid = this.updateCardIcon(this.brojKarticeReg);
    if (!isCardValid) {
      return;
    }
      ...
      if (user) {
        ...
        this.userService.getUserByEmail(this.emailReg).subscribe(user => {
          if (user) {
            ...
            this.userService.newDecorator(this.korisnickoImeReg, this.lozinkaReg, this.imeReg, this.prezimeReg, this.adresaReg, this.polReg, 
            this.telefonReg, this.emailReg, this.profilnaSlikaReg, this.brojKarticeReg).subscribe(resp => {
              alert('Uspešno ste poslali dodali novog dekoratera!');
